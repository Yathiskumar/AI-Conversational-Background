...
export default function SpeechInput({ selectedLang }: { selectedLang: string }) {
	...
	const getLangConfig = useCallback(() => {
		switch (selectedLang) {
			case "hi":
				return { voice: "hi-IN", prompt: "Respond only in Hindi." };
			default:
				return { voice: "en-US", prompt: "Respond only in English." };
		}
	}, [selectedLang]);

	// speak helper
	const speak = useCallback(
		(msg: string, lang: string) => {
			stopSpeaking();
	...
	const startChat = useCallback(async () => {
		...
		speak(aiReply, voice);
	}, [getLangConfig, speak, stopListening, stopSpeaking]);
